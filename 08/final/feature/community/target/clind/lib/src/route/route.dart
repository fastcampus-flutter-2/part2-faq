  ...
  static Widget findScreen(Uri uri) {
    ...
    switch (route) {
      case CommunityRoute.community:
        final String type = uri.queryParameters['type'] ?? Modular.args.data?['type'] as String? ?? '';
        return CommunityBlocProvider(
          child: CommunityScreen(
            type: CommunityTabType.decode(type),
          ),
        );
      case CommunityRoute.post:
        final String id = uri.queryParameters['id'] ?? Modular.args.data?['id'] as String? ?? '';
